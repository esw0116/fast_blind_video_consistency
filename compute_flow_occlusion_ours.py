...
if __name__ == "__main__":
    ...
    ### load image list
    input_dir = os.path.join(opts.data_dir, opts.dataset, opts.phase)
    print(input_dir)
    video_list = sorted(glob.glob(os.path.join(input_dir, '*')))
    ...
    for video_dir in video_list:
        ...
        for t in range(len(frame_list) - 1):
            ...
            file_name = os.path.splitext(os.path.basename(frame_list[t]))[0]
            ...
            fw_flow = utils.resize_flow(fw_flow, W_out = W_sc, H_out = H_sc) 
            bw_flow = utils.resize_flow(bw_flow, W_out = W_sc, H_out = H_sc)
            ### compute occlusion
            fw_occ = utils.detect_occlusion(bw_flow, fw_flow)

            ### save flow
            output_flow_filename = os.path.join(fw_flow_dir, "{}.flo".format(file_name))
            if not os.path.exists(output_flow_filename):
                utils.save_flo(fw_flow, output_flow_filename)
        
            ### save occlusion map
            output_occ_filename = os.path.join(fw_occ_dir, "{}.png".format(file_name))
            if not os.path.exists(output_occ_filename):
                utils.save_img(fw_occ, output_occ_filename)

            ### save rgb flow
            output_filename = os.path.join(fw_rgb_dir, "{}.png".format(file_name))
